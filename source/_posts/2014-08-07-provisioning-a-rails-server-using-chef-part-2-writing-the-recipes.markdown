...
By default, Chef inspects the process table to see if a service is running, but since there is an upstart job for SSH, we can use the `service ssh status` command to see if it is running. That's why we use the `supports` attribute to tell Chef to use the `status` command instead. And while we're at it, we also give Chef permission to use the `restart` command to restart SSH.
