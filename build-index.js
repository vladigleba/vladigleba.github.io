...
/**
 * Clean heading - remove # symbols and emphasis
 */
function cleanHeading(text) {
  return cleanLinks(removeEmphasis(
      text.replace(/^#+\s*/, '') // remove leading #
    )
  );
}

/**
 * Clean blockquote - remove > and handle citations
 */
function cleanBlockquote(text) {
  return cleanLinks(cleanLiquidTags(removeEmphasis(
        text.replace(/^>\s*/gm, '') // remove > at start of lines
      )
    )
  );
}
...
/**
 * Clean paragraph - handle liquid tags, emphasis, and inline footnote references
 */
function cleanParagraph(text) {
  return cleanLinks(cleanLiquidTags(removeEmphasis(
        text.replace(/\[\^\d+\]/g, '') // remove inline [^1] markers
      )
    )
  );
}
...
/**
 * Clean list - remove list markers and emphasis
 */
function cleanList(text) {
  return cleanLinks(cleanLiquidTags(removeEmphasis(
        text
          ...
          .replace(/\[\^\d+\]/g, '') // remove inline [^1] markers
      )
    )
  );
}
...
/**
 * Clean footnote - remove footnote syntax
 */
function cleanFootnote(text) {
  return cleanLinks(cleanLiquidTags(removeEmphasis(
        text.replace(/^\[\^\d+\]:\s*/, '') // remove [^1]: 
      )
    )
  );
}
 ...
 * Each block gets a data-search-id for precise search result linking
 * IDs match the format generated by Eleventy transform
 */
function extractContentBlocks(bodyText) {
  ...
  const saveBlock = () => {
    ...
    // type-specific cleaning and ID assignment
    switch(blockType) {
      case 'heading':
